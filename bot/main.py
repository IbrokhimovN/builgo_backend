...
Handles user registration for Telegram Mini App.
Deterministic flow:
1. Get telegram_id
2. Call GET /api/check-seller/
3. If seller → open Seller Mini App
4. Else → collect name + phone → POST /api/customers/ → open Customer Mini App

NO role selection. NO authentication. NO sessions.
"""
...
from config import BOT_TOKEN, MINI_APP_URL, CUSTOMER_ENDPOINT, CHECK_SELLER_ENDPOINT
...
# Conversation states (no role selection)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    3. Else → start customer registration (ask name)
    """
    ...
    # Not a seller → start customer registration
...
async def receive_phone(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info(
        "Saving customer: telegram_id=%s, name=%s %s, phone=%s",
